...
一个本地测试中的 Galgame 视听档案站，用于按游戏整理封面、简介、CG 和 BGM。当前项目是单页静态应用 + JSON 数据驱动；BGM 已基本迁移到 Cloudflare R2/Worker，CG 展示图开始使用 PicX 图床，游戏封面暂时保留在项目目录中。
...
- BGM：详情页以试听卡片展示，底部播放器支持播放/暂停、上一首/下一首、关闭、进度条、时间显示、播放结束自动下一首；播放器在单页路由切换时保持常驻，播放失败时会在播放器中提示。
- 主题：日间/夜间切换，保存在浏览器本地。
...
2. 继续微调播放器观感和移动端布局。
3. 清理旧的 `assets/js/home.js` / `assets/js/game.js`，确认没有旧页面依赖后再删除。
